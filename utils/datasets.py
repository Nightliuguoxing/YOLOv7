...
img_formats = ['bmp', 'jpg', 'jpeg', 'png', 'tif', 'tiff', 'dng', 'webp', 'mpo']  # acceptable images suffixes
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            #print(f'images {self.count}/{self.nf} {path}: ', end='')
...
def img2label_paths(img_paths):
    # Define anns paths as a function of images paths
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                nc += 1
                print(f'{prefix}WARNING: Ignoring corrupted images and/or label {im_file}: {e}')
...
def load_image(self, index):
    # loads 1 images from dataset, returns img, original hw, resized hw
    img = self.imgs[index]
    if img is None:  # not cached
        ...
        r = self.img_size / max(h0, w0)  # resize images to img_size
...
def hist_equalize(img, clahe=True, bgr=False):
    # Equalize histogram on BGR images 'img' with img.shape(n,m,3) and range 0-255
...
def pastein(image, labels, sample_labels, sample_images, sample_masks):
    ...
    # create random masks
    scales = [0.75] * 2 + [0.5] * 4 + [0.25] * 4 + [0.125] * 4 + [0.0625] * 6  # images size fraction
    for s in scales:
        ...
        if (ioa < 0.30).all() and len(sample_labels) and (xmax > xmin+20) and (ymax > ymin+20):  # allow 30% obscuration of existing labels
            ...
            #print(images[ymin:ymax, xmin:xmax].shape)
...
def autosplit(path='../coco', weights=(0.9, 0.1, 0.0), annotated_only=False):
    ...
    """
    path = Path(path)  # images dir
    files = sum([list(path.rglob(f"*.{img_ext}")) for img_ext in img_formats], [])  # images files only
    n = len(files)  # number of files
    ...
    for i, img in tqdm(zip(indices, files), total=n):
        if not annotated_only or Path(img2label_paths([str(img)])[0]).exists():  # check anns
