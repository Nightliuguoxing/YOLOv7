...
def smooth_BCE(eps=0.1):  # https://github.com/ultralytics/yolov3/issues/238#issuecomment-598028441
    # return positive, negative anns smoothing BCE targets
    return 1.0 - 0.5 * eps, 0.5 * eps


class BCEBlurWithLogitsLoss(nn.Module):
    # BCEwithLogitLoss() with reduced missing anns effects.
    ...
    def forward(self, pred, true):
        ...
        dx = pred - true  # reduce only missing anns effects
        # dx = (pred - true).abs()  # reduce missing anns and false anns effects
...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        # Class anns smoothing https://arxiv.org/pdf/1902.04103.pdf eqn 3
...
class ComputeLossOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        # Losses
        for i, pi in enumerate(p):  # layer index, layer predictions
            b, a, gj, gi = bs[i], as_[i], gjs[i], gis[i]  # images, anchor, gridy, gridx
            tobj = torch.zeros_like(pi[..., 0], device=device)  # target obj
            ...
            if n:
                ...
                lbox += (1.0 - iou).mean()  # iou loss
                CIoU or DIoU or GIoU or EIoU or SIoU
                '''
                # FIXME: FocalLoss_='CIoU', CIoU=True Keep Agreement
                iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False,
                               FocalLoss_='CIoU',
                               CIoU=True, DIoU=False, GIoU=False, EIoU=False, SIoU=False)  # iou(prediction, target)
                iou_ori = iou[0]
                iou_f = iou[1]
                if len(iou) >= 2:
                    lbox += (iou_f.detach() * (1 - iou_ori)).mean()
                    iou = iou_ori
                else:
                    lbox += (1.0 - iou).mean()  # iou loss
                # Objectness
...
class ComputeLossAuxOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            pi_aux = p[i+self.nl]
            b, a, gj, gi = bs[i], as_[i], gjs[i], gis[i]  # images, anchor, gridy, gridx
            b_aux, a_aux, gj_aux, gi_aux = bs_aux[i], as_aux_[i], gjs_aux[i], gis_aux[i]  # images, anchor, gridy, gridx
            tobj = torch.zeros_like(pi[..., 0], device=device)  # target obj
