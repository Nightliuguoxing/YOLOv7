...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            conf = None if labels else image_targets[:, 6]  # check for confidence presence (anns vs pred)

            if boxes.shape[1]:
                if boxes.max() <= 1.01:  # if normalized with tolerance 0.01
                    boxes[[0, 2]] *= w  # scale to pixels
                    boxes[[1, 3]] *= h
                elif scale_factor < 1:  # absolute coords need scale if images scales
                    boxes *= scale_factor
            boxes[[0, 2]] += block_x
            boxes[[1, 3]] += block_y
    ...
    for f in sorted(glob.glob('results*.txt') + glob.glob('../../Downloads/results*.txt')):
        ...
        for i in range(5):
            for j in [i, i + 5]:
                ...
                # ax[i].plot(x, np.gradient(y_smooth), marker='.', anns=s[j])
